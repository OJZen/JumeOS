...
Item {
    ...
    readonly property var current: entries[selected] || ({})
    readonly property string heading: kind === "neo" ? "Neo 模拟器管理" : kind === "ports" ? "PortMaster 管理" : "USB 手柄配置"
    readonly property real detailX: kind === "usb" ? 0 : Ui.Theme.detailX
    readonly property real detailWidth: width - detailX
    ...
    function activate() {
        if (tools.busy || !rows[row] || !rows[row].enabled) return
        const entry = rows[row]
        if (entry.toggle) tools.choose(entry.key, !tools.settings[entry.key])
        else if (entry.key === "usbDeadzone") choices.present(details.currentItem, ["0%", "5%", "10%", "15%", "20%", "25%", "30%"], tools.settings.usbDeadzone / 5, "摇杆死区")
    ...
    function dispatch(action, repeated) {
        ...
        if (sidebar) {
            ...
            else if ((action === "accept" || action === "right") && entries.length) { sidebar = false; row = 0 }
        } else {
    ...
    onKindChanged: { catalogOpen = false; selected = 0; row = 0; sidebar = kind !== "usb" }
    Ui.PageHeader {
        width: parent.width; title: view.heading; visible: !view.catalogOpen
        actionText: view.kind === "ports" && view.tools.catalogAvailable ? "游戏目录" : ""
        onActionRequested: view.openCatalog()
        iconName: view.kind === "neo" ? "cartridge" : view.kind === "ports" ? "ports" : "usb"
        backVisible: true; onBackRequested: view.backRequested()
        subtitle: view.tools.error || (view.tools.busy ? "正在处理…" : view.kind === "usb" ? view.facts.status || "USB 输出尚未启用" : view.current.status || "")
        error: !!view.tools.error; busy: view.tools.busy
    }
    Ui.ListView {
        id: games; objectName: "toolGames"; y: Ui.Theme.contentY; width: Ui.Theme.sidebarWidth; height: parent.height - y
        visible: view.kind !== "usb" && !view.catalogOpen; model: view.entries; currentIndex: view.selected
        delegate: SettingRow {
            ...
            onActivated: { view.selected = index; view.sidebar = false; view.row = 0 }
        }
    }
    Ui.ListView {
        id: details; objectName: "toolRows"; visible: !view.catalogOpen; x: view.detailX; y: Ui.Theme.contentY; width: view.detailWidth; height: parent.height - y
        ...
        delegate: SettingRow {
            ...
            onActivated: { view.sidebar = false; view.row = index; view.activate() }
        }
    }
    Item {
        visible: !view.catalogOpen
        y: Ui.Theme.contentY; width: parent.width; height: parent.height - y; clip: true
        Ui.FocusFrame {
            x: view.sidebar ? 0 : view.detailX
            y: view.sidebar ? (games.currentItem ? games.currentItem.y - games.contentY : 0) : (details.currentItem ? details.currentItem.y - details.contentY : 0)
            width: view.sidebar ? games.rowWidth : details.rowWidth; height: Ui.Theme.rowHeight
            active: view.navigationActive && !choices.visible && !view.tools.busy
