#!/bin/bash
# Source-built GTA and original Mono engines, isolated input and writable state; no original launchers.
set -Eeuo pipefail
[[ -f /.dockerenv && -d /code && -d /content/ports && -x /gta-source/re3 && -x /gta-source/reVC && -x /mono/bin/mono ]] || exit 2
mkdir -p /out/ports
