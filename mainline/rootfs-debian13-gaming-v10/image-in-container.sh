...
readonly PRODUCT_DOC_SOURCE=$SOURCE/mainline/rootfs-debian13-gaming-v10/PRODUCT.md

readonly BASE_SYSTEMS_SOURCE_SHA256=1f451723d4ce1062157eb0e041ff4c098c5adea98c61e87ddf3cb784dfe630f0
readonly ARCADE_FRAGMENT_SHA256=1fbe3e50aaca9c84b2bd2f0a66b41f170c5adcbde8580c8215b60b8c3fbc4711
readonly CPS1_FRAGMENT_SHA256=5ff352b7e5966883a93539a5af92baf4e594d50f881f7ec146c3edb7d110c138
readonly BASE_LINKS_SOURCE_SHA256=bd5e496ab9bb54f0b9d45b461262b70fd5758d763ea4ff4fb9d06c26c936d73d
readonly ARCADE_LINK_SHA256=058d05441005abd21fced96259c949f469bba10f8bb455d6eb21bcfc05d31e6f
readonly CPS1_LINK_SHA256=c994ef9ffe8e41e9f33e6b6beb15feae7b0e5c59f54955689b95a8f69549e904
...
readonly PRODUCT_DOC_SHA256=4b894d261418733367474ef7d0ac55ce0620b8605169c63b5d7c675be9fd5f0f
...
verify_inputs() {
  ...
  expect_size_sha256 "$INPUTS/r46h-firstboot.v09" 835 \
    "$BASE_FIRSTBOOT_SHA256" 'v0.9 firstboot input'
  expect_size_sha256 "$INPUTS/r46h-rootfs-smoke.v09" 8294 \
    "$BASE_ROOTFS_SMOKE_SHA256" 'v0.9 smoke input'
  members=$(find "$INPUTS" -mindepth 1 -maxdepth 1 -printf '%f\n' | LC_ALL=C sort)
  [[ $members == $'r46h-firstboot.v09\nr46h-rootfs-smoke.v09' ]] || \
    die 'unexpected product input set'

  expect_sha256 "$BASE_SYSTEMS_SOURCE" "$BASE_SYSTEMS_SOURCE_SHA256" \
    'base ES-DE systems source'
  expect_sha256 "$ARCADE_FRAGMENT_SOURCE" "$ARCADE_FRAGMENT_SHA256" \
    'Arcade system fragment'
  expect_sha256 "$CPS1_FRAGMENT_SOURCE" "$CPS1_FRAGMENT_SHA256" \
    'CPS1 system fragment'
  expect_sha256 "$BASE_LINKS_SOURCE" "$BASE_LINKS_SOURCE_SHA256" \
    'base system links source'
  expect_sha256 "$ARCADE_LINK_SOURCE" "$ARCADE_LINK_SHA256" \
    'Arcade link fragment'
  expect_sha256 "$CPS1_LINK_SOURCE" "$CPS1_LINK_SHA256" 'CPS1 link fragment'
...
verify_base() {
  ...
  path_is_absent "$CPS1_LINK" || die 'base already exposes CPS1'
...
prepare_system_files() {
  ...
  ' "$BASE_SYSTEMS_SOURCE" "$ARCADE_FRAGMENT_SOURCE" "$CPS1_FRAGMENT_SOURCE" \
    > "$WORK/es-systems.v10"
  expect_sha256 "$WORK/es-systems.v10" "$ES_DE_SYSTEMS_SHA256" 'v0.10 ES-DE systems'
  awk '1' "$BASE_LINKS_SOURCE" "$ARCADE_LINK_SOURCE" "$CPS1_LINK_SOURCE" \
    > "$WORK/system-links.v10"
  expect_sha256 "$WORK/system-links.v10" "$SYSTEM_LINKS_SHA256" 'v0.10 system links'
}
...
prepare_runner() {
  ...
  ' "$RUNNER_SOURCE" > "$WORK/r46h-es-de-ui.v10"
  chmod 0755 "$WORK/r46h-es-de-ui.v10"
  expect_sha256 "$WORK/r46h-es-de-ui.v10" "$ES_DE_RUNNER_SHA256" 'v0.10 ES-DE runner'
  ...
  sed \
    ...
    "$SCREENSHOT_SOURCE" > "$WORK/r46h-screenshot.v10"
  chmod 0755 "$WORK/r46h-screenshot.v10"
  expect_sha256 "$WORK/r46h-screenshot.v10" "$SCREENSHOT_SHA256" 'v0.10 screenshot helper'
}

prepare_identity_tools() {
  sed 's/debian13-p2-gaming-v0\.9/debian13-p2-gaming-v0.10/g' \
    "$INPUTS/r46h-firstboot.v09" > "$WORK/r46h-firstboot.v10"
  chmod 0755 "$WORK/r46h-firstboot.v10"
  expect_sha256 "$WORK/r46h-firstboot.v10" "$FINAL_FIRSTBOOT_SHA256" 'v0.10 firstboot tool'
  sed \
    -e "s/$BASE_FS_UUID/$FS_UUID/g" \
    -e 's/debian13-p2-gaming-v0\.9/debian13-p2-gaming-v0.10/g' \
    "$INPUTS/r46h-rootfs-smoke.v09" > "$WORK/r46h-rootfs-smoke.v10"
  chmod 0755 "$WORK/r46h-rootfs-smoke.v10"
  expect_sha256 "$WORK/r46h-rootfs-smoke.v10" "$FINAL_ROOTFS_SMOKE_SHA256" \
    'v0.10 rootfs smoke tool'
}

prepare_receipts() {
  cat > "$WORK/es-de-receipt.v10" <<EOF
...
composition=offline-cps1-successor-p2-v0.10
EOF
  chmod 0600 "$WORK/es-de-receipt.v10"
  expect_sha256 "$WORK/es-de-receipt.v10" "$ES_DE_RECEIPT_SHA256" 'v0.10 ES-DE receipt'
...
prepare_manifests() {
  : > "$DIRECT"
  : > "$LINKS"
  add_direct "$WORK/es-systems.v10" "$SYSTEMS" 0100644 0 0 replace
  add_direct "$WORK/r46h-es-de-ui.v10" "$RUNNER" 0100755 0 0 replace
  add_direct "$WORK/r46h-screenshot.v10" "$SCREENSHOT" 0100755 0 0 replace
  add_direct "$WORK/es-de-receipt.v10" "$ES_DE_RECEIPT" 0100600 0 0 replace
  add_direct "$WORK/system-links.v10" "$ES_DE_STATE/system-links.tsv" 0100400 0 0 replace
  add_direct "$PRODUCT_DOC_SOURCE" "$ES_DE_STATE/README.md" 0100400 0 0 replace
  add_direct "$WORK/r46h-firstboot.v10" "$FIRSTBOOT" 0100755 0 0 replace
  add_direct "$WORK/r46h-rootfs-smoke.v10" "$ROOTFS_SMOKE" 0100755 0 0 replace
  add_direct "$WORK/CONSOLIDATED-RECEIPT" "$RECEIPT" 0100644 0 0 replace
  printf '%s\t%s\t%s\t%s\n' "$CPS1_LINK" /roms/cps1 1000 1000 >> "$LINKS"
}
...
verify_unchanged_files() {
  ...
  printf 'R46H_V10_SYSTEMD_VERIFY_RESULT=pass\n' > "$EVIDENCE/SYSTEMD-VERIFY.txt"
  printf 'R46H_V10_UDEV_VERIFY_RESULT=pass\n' > "$EVIDENCE/UDEV-VERIFY.txt"
}
...
verify_final() {
  ...
  printf 'R46H_V10_E2FSCK_RESULT=pass\n' >> "$EVIDENCE/E2FSCK.txt"
  ...
  printf '%s\n' "$stat_output" >> "$EVIDENCE/DEBUGFS-V10.txt"
  grep -Fq 'Fast link dest: "/roms/cps1"' <<< "$stat_output" || \
    die 'CPS1 link target mismatch'
  grep -Eq 'User: +1000 +Group: +1000 ' <<< "$stat_output" || \
    die 'CPS1 link owner mismatch'
  verify_unchanged_files

  bash -n "$WORK/r46h-es-de-ui.v10" "$WORK/r46h-screenshot.v10" \
    "$WORK/r46h-firstboot.v10" "$WORK/r46h-rootfs-smoke.v10"
...
R46H_V10_PRODUCT artifact_id=$ARTIFACT_ID frontend=ES-DE-3.4.1-r51 systems=9 media_links=3417
R46H_V10_PRODUCT fbneo_full=$FBNEO_FULL_CORE_SHA256 arcade_samples=11 cps1_samples=48 roms=external-read-only-p3
R46H_V10_PRODUCT_VERIFY_RESULT=pass
EOF
  printf 'R46H_V10_IMAGE_VERIFY_RESULT=pass\n' >> "$EVIDENCE/DEBUGFS-V10.txt"
}
...
main() {
  ...
  verify_final
  printf 'PASS: R46H Debian 13 gaming p2 v0.10 image %s completed.\n' "$ACTION"
}
