...
verify_inputs() {
  local exclusive_line members system_line
  [[ -d $INPUTS && ! -L $INPUTS ]] || die 'product inputs are missing'
  expect_size_sha256 "$INPUTS/es-de-systems.v17.xml" 6452 "$ES_DE_SYSTEMS_SHA256" 'ES-DE systems input'
  expect_size_sha256 "$INPUTS/r46h-es-de-ui.v17" 10026 "$ES_DE_RUNNER_SHA256" 'ES-DE runner input'
  expect_size_sha256 "$INPUTS/r46h-screenshot.v17" 11371 "$SCREENSHOT_SHA256" 'screenshot input'
  expect_size_sha256 "$INPUTS/r46h-firstboot.v17" 837 "$FINAL_FIRSTBOOT_SHA256" 'firstboot input'
  expect_size_sha256 "$INPUTS/r46h-rootfs-smoke.v17" 8295 "$FINAL_ROOTFS_SMOKE_SHA256" 'rootfs smoke input'
  expect_size_sha256 "$INPUTS/es-de-receipt.v17" 2597 "$ES_DE_RECEIPT_SHA256" 'ES-DE receipt input'
  members=$(find "$INPUTS" -mindepth 1 -maxdepth 1 -printf '%f\n' | LC_ALL=C sort)
  [[ $members == $'es-de-receipt.v17\nes-de-systems.v17.xml\nr46h-es-de-ui.v17\nr46h-firstboot.v17\nr46h-rootfs-smoke.v17\nr46h-screenshot.v17' ]] || \
    die 'unexpected product input set'
  bash -n "$INPUTS/r46h-es-de-ui.v17" "$INPUTS/r46h-screenshot.v17" \
    "$INPUTS/r46h-firstboot.v17" "$INPUTS/r46h-rootfs-smoke.v17"
  [[ $(grep -Fxc '<loadExclusive/>' "$INPUTS/es-de-systems.v17.xml") == 1 ]] || \
    die 'ES-DE exclusive marker is missing or misplaced'
  exclusive_line=$(grep -nFx '<loadExclusive/>' "$INPUTS/es-de-systems.v17.xml" | cut -d: -f1)
  system_line=$(grep -nFx '<systemList>' "$INPUTS/es-de-systems.v17.xml" | cut -d: -f1)
  (( exclusive_line == 2 && system_line > exclusive_line )) || \
    die 'ES-DE system list root is misplaced'
  [[ $(grep -Fc '<system>' "$INPUTS/es-de-systems.v17.xml") == 13 ]] || \
    die 'ES-DE system count mismatch'
  ! grep -Fq '<name>dreamcast</name>' "$INPUTS/es-de-systems.v17.xml" || \
...
verify_base() {
  ...
  [[ $(grep -Fc '<system>' "$WORK/hash-systems") == 13 ]] || die 'base system count mismatch'
  ! grep -Fq '<name>dreamcast</name>' "$WORK/hash-systems" || \
    die 'base unexpectedly advertises Dreamcast'
  ! grep -Fq '<loadExclusive/>' "$WORK/hash-systems" || \
    die 'base unexpectedly uses exclusive custom systems'
...
prepare_receipt() {
  ...
  replace_line "$WORK/CONSOLIDATED-RECEIPT" \
    'artifact_id=debian13-p2-gaming-v0.16' "artifact_id=$ARTIFACT_ID"
  replace_line "$WORK/CONSOLIDATED-RECEIPT" \
    'source_git_commit=42346b61dcf7837cc46b49d4bd6f3da4f5d7802b' "source_git_commit=$SOURCE_GIT_COMMIT"
  replace_line "$WORK/CONSOLIDATED-RECEIPT" \
    'source_git_tree=c22cb7f0259a84fda812ff7e2506af19fe2165f5' "source_git_tree=$SOURCE_GIT_TREE"
  replace_line "$WORK/CONSOLIDATED-RECEIPT" \
    'source_manifest_sha256=1a2c22caa7c814e527e1374fccb077183b4377568a3b60fd5fb189eeb024a948' \
    "source_manifest_sha256=$SOURCE_MANIFEST_SHA256"
  replace_line "$WORK/CONSOLIDATED-RECEIPT" \
    'successor_base_artifact_id=debian13-p2-gaming-v0.15' \
    'successor_base_artifact_id=debian13-p2-gaming-v0.16'
  replace_line "$WORK/CONSOLIDATED-RECEIPT" \
    'successor_base_image_sha256=a69c2dafeae76f37a6e582bfdf4887d5714b82a4cf5b3dc7a84e29f36be6e893' \
    "successor_base_image_sha256=$BASE_IMAGE_SHA256"
  replace_line "$WORK/CONSOLIDATED-RECEIPT" \
    'base_consolidated_receipt_sha256=7a3d4fa07e4a34f9313e1fd4a84c66389cbb7a9cfc740659f241461e4a01524d' \
  ...
  printf '%s\n' 'es_de_systems_scope=exclusive-custom' >> "$WORK/CONSOLIDATED-RECEIPT"
...
apply_overlay() {
  ...
  : > "$commands"
  emit_replace "$INPUTS/es-de-systems.v17.xml" "$SYSTEMS" 0100644 0 0 >> "$commands"
  emit_replace "$INPUTS/r46h-es-de-ui.v17" "$RUNNER" 0100755 0 0 >> "$commands"
  emit_replace "$INPUTS/r46h-screenshot.v17" "$SCREENSHOT" 0100755 0 0 >> "$commands"
  emit_replace "$INPUTS/r46h-firstboot.v17" "$FIRSTBOOT" 0100755 0 0 >> "$commands"
  emit_replace "$INPUTS/r46h-rootfs-smoke.v17" "$ROOTFS_SMOKE" 0100755 0 0 >> "$commands"
  emit_replace "$INPUTS/es-de-receipt.v17" "$ES_DE_RECEIPT" 0100600 0 0 >> "$commands"
  emit_replace "$WORK/CONSOLIDATED-RECEIPT" "$RECEIPT" 0100644 0 0 >> "$commands"
  for directory in /etc/r46h /usr/local/sbin /usr/local/bin \
...
verify_file() {
  ...
  printf '%s\n' "$stat_output" >> "$EVIDENCE/DEBUGFS-V17.txt"
...
verify_retained() {
  ...
  for pair in \
    ...
    "$OVERRIDE_RULE:$ALSA_OVERRIDE_RULE_SHA256" \
    "$REMOTE_INPUT:$REMOTE_INPUT_SHA256"; do
    index=$((index + 1))
    path=${pair%%:*}
    expected=${pair#*:}
  ...
  printf 'R46H_V17_SYSTEMD_VERIFY_RESULT=pass\n' > "$EVIDENCE/SYSTEMD-VERIFY.txt"
  printf 'R46H_V17_UDEV_VERIFY_RESULT=pass\n' > "$EVIDENCE/UDEV-VERIFY.txt"
}
...
verify_final() {
  local exclusive_line stat_output machine_id system_line
  ...
  printf 'R46H_V17_E2FSCK_RESULT=pass\n' >> "$EVIDENCE/E2FSCK.txt"
  ...
  : > "$EVIDENCE/DEBUGFS-V17.txt"
  verify_file "$INPUTS/es-de-systems.v17.xml" "$SYSTEMS" 0100644 0 0 systems
  verify_file "$INPUTS/r46h-es-de-ui.v17" "$RUNNER" 0100755 0 0 runner
  verify_file "$INPUTS/r46h-screenshot.v17" "$SCREENSHOT" 0100755 0 0 screenshot
  verify_file "$INPUTS/r46h-firstboot.v17" "$FIRSTBOOT" 0100755 0 0 firstboot
  verify_file "$INPUTS/r46h-rootfs-smoke.v17" "$ROOTFS_SMOKE" 0100755 0 0 smoke
  verify_file "$INPUTS/es-de-receipt.v17" "$ES_DE_RECEIPT" 0100600 0 0 esde-receipt
  ...
  [[ $(grep -Fxc '<loadExclusive/>' "$WORK/final-systems") == 1 ]] || \
    die 'final exclusive marker is missing or misplaced'
  exclusive_line=$(grep -nFx '<loadExclusive/>' "$WORK/final-systems" | cut -d: -f1)
  system_line=$(grep -nFx '<systemList>' "$WORK/final-systems" | cut -d: -f1)
  (( exclusive_line == 2 && system_line > exclusive_line )) || \
    die 'final system list root is misplaced'
  ...
  grep -Fq 'release=debian13-p2-gaming-v0.17' "$WORK/final-firstboot" || \
    die 'firstboot version mismatch'
  grep -Fq "readonly EXPECTED_ROOT_UUID=$FS_UUID" "$WORK/final-smoke" || \
    die 'rootfs smoke UUID mismatch'
  grep -Fq 'debian13-p2-gaming-v0.17' "$WORK/final-smoke" || die 'rootfs smoke version mismatch'
...
R46H_V17_PRODUCT artifact_id=$ARTIFACT_ID frontend=ES-DE-3.4.1-r51 systems=13 media_links=6220
R46H_V17_PRODUCT remote_input_hold_ms=10 screenshot_guard=anchored-command-line-one-plus-one dreamcast_menu=hidden systems_scope=exclusive-custom
R46H_V17_PRODUCT dreamcast_core=retained dreamcast_content=retained roms=external-read-only-p3
R46H_V17_PRODUCT_VERIFY_RESULT=pass
EOF
  printf 'R46H_V17_IMAGE_VERIFY_RESULT=pass\n' >> "$EVIDENCE/DEBUGFS-V17.txt"
}
...
main() {
  ...
  verify_final
  printf 'PASS: R46H Debian 13 gaming p2 v0.17 image %s completed.\n' "$ACTION"
}
