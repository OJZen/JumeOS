#!/usr/bin/env python3
"""Build a full offline BOOT p1 image containing a one-shot v0.9 candidate."""
...
BUILD_ID = "v0.9-adc-joystick-fix"
RELEASE = "6.12.99-r46h-mainline-v0.9-adc-joystick-fix"
ARTIFACT_ID = "r46h-v0.9-p1-one-shot"
SOURCE_DATE_EPOCH = 1_785_369_600
OUTPUT_IMAGE_NAME = "p1-v0.9-one-shot.img"
V09_IMAGE_NAME = "Image.mainline-v0.9-adc-joystick-fix.gz"
V09_BOOT_NAME = "boot.ini.v0.9-adc-joystick-fix"
...
BUILDER_RELPATH = "mainline/scripts/build-v09-p1-candidate.py"
...
BUNDLE_NAME = "r46h-easyroms-v0.9-adc-joystick-fix"
PACKAGE_NAME = "r46h-mainline-test-v0.9-adc-joystick-fix"
...
REMOVED_FILES: dict[str, tuple[int, str]] = {
    "Image.mainline-test.gz": (
        14_919_642,
        "fb8b8d39262e087af5a9efa73c4cb8f5f7d0559f9d2b0d30acc4dd9156140abb",
    ),
    "rk3326-r46h-mainline-test.dtb": (
        49_448,
        "074bb3121fafe5df96872628ab22db03ffd63adf53c990e779e5a0e2f54b7a2b",
    ),
    "boot.ini.test": (
        1_327,
        "921f439aa602bb8bc84ffe39cdb00a9ffd03dab1bed1d8960d2e30fdff4703cc",
    ),
...
def render_v09_boot(
    ...
    *,
    ...
    replacements = {
        ...
        b"@IMAGE_NAME@": V09_IMAGE_NAME.encode(),
        ...
        b"@DTB_NAME@": V08_DTB_NAME.encode(),
    ...
    try:
        ...
        raise BuildError("rendered v0.9 boot candidate is not ASCII") from exc
    required_fragments = (
    ...
    if any(fragment not in text for fragment in required_fragments):
        raise BuildError("rendered v0.9 boot candidate safety contract mismatch")
    if "saveenv" in text:
        raise BuildError("rendered v0.9 boot candidate must not persist U-Boot state")
    return result
...
def require_modules_only_bundle(repo_root: Path, bundle_dir: Path) -> dict[str, Any]:
    ...
    required_manifest = {
        ...
        "boot_image": V09_IMAGE_NAME,
        "boot_dtb": f"rk3326-r46h-mainline-{BUILD_ID}.dtb",
        "boot_candidate": V09_BOOT_NAME,
    ...
    required_names = {
        ...
        V09_IMAGE_NAME,
        V09_BOOT_NAME,
        f"rk3326-r46h-mainline-{BUILD_ID}.dtb",
        PACKAGE_TAR_NAME,
        "bootstrap-target.sh",
        "install-modules.sh",
        "target-common.sh",
    ...
    if (
        ...
        raise BuildError("canonical package identity does not match the v0.9 p1 contract")
    if (
    ...
    if len(bundle_dtb) != V08_DTB_SIZE or hashlib.sha256(bundle_dtb).hexdigest() != V08_DTB_SHA256:
        raise BuildError("v0.9 package DTB is not byte-identical to the preserved v0.8 DTB")
    if (
...
def revalidate_canonical_sources(
    ...
    if drifted:
        raise BuildError(f"canonical v0.9 sources drifted during p1 build: {drifted}")
    return current
...
def validate_exact_diff(
    ...
    require_file_identity(after, added_files, "added v0.9 file")
...
mdel -i "$image" ::/Image.mainline-test.gz
mdel -i "$image" ::/rk3326-r46h-mainline-test.dtb
mdel -i "$image" ::/boot.ini.test
mdel -i "$image" ::/boot.ini.v0.2-known-good
mcopy -o -m -i "$image" /work/v09-image.gz ::/Image.mainline-v0.9-adc-joystick-fix.gz
mcopy -o -m -i "$image" /work/v09-boot.ini ::/boot.ini.v0.9-adc-joystick-fix
...
def build_candidate(
    ...
    added_files = {
        V09_IMAGE_NAME: (len(compressed_image), hashlib.sha256(compressed_image).hexdigest()),
        V09_BOOT_NAME: (len(boot_data), hashlib.sha256(boot_data).hexdigest()),
    }
    source_state = {
    ...
    with tempfile.TemporaryDirectory(prefix=".r46h-v09-p1-build.", dir=cache_root) as temporary:
        work = Path(temporary)
        work.chmod(0o700)
        ...
        require_file_identity(before, REMOVED_FILES, "retired v0.2 file")
        require_file_identity(before, PRESERVED_V08_ANCHORS, "required v0.8 anchor")
        for new_path in added_files:
            if new_path in before:
                raise BuildError(f"v0.9 destination already exists in input clone: {new_path}")

        gzip_path = work / "v09-image.gz"
        ...
        (receipt / V09_IMAGE_NAME).write_bytes(gzip_path.read_bytes())
        (receipt / V09_BOOT_NAME).write_bytes(boot_data)
        for path in (receipt / V09_IMAGE_NAME, receipt / V09_BOOT_NAME):
            path.chmod(0o600)
        ...
        (stage / "SHA256SUMS").write_text(
            ...
            f"{added_files[V09_IMAGE_NAME][1]}  receipt/{V09_IMAGE_NAME}\n"
            f"{added_files[V09_BOOT_NAME][1]}  receipt/{V09_BOOT_NAME}\n",
            encoding="utf-8",
        )
    ...
    print("PASS: offline full-p1 v0.9 one-shot candidate built.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "mainline/out/r46h-easyroms-v0.9-adc-joystick-fix"
        ),
    )
...
def main() -> int:
    try:
        ...
        args = parse_args()
        output_parent = args.output_parent or repo_root / "mainline/out/r46h-v09-p1-candidates"
        build_candidate(
            repo_root,
            args.input_p1,
            output_parent,
