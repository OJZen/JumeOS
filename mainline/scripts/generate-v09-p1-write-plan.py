#!/usr/bin/env python3
"""Generate a device-path-bound Card Agent plan for the audited v0.9 p1 image."""
...
IMAGE_NAME = "p1-v0.9-one-shot.img"
OPERATION_ID = "write-v09-one-shot-boot-p1"
...
def load_candidate(repo_root: Path, candidate_dir: Path) -> tuple[Path, str, str]:
    ...
    if (
        ...
        or status.get("v08_dtb_reused") is not True
...
def build_write_plan(
    ...
    plan = {
        ...
        "operations": [
            {
                "id": OPERATION_ID,
                "description": "Write full R46H v0.9 one-shot BOOT p1 candidate image",
                "partition": "boot",
...
def create_plan(
    ...
    output_parent = output_parent or repo_root / "mainline/out/r46h-v09-p1-deploy-plans"
    ...
    plan_dir = Path(tempfile.mkdtemp(prefix=".r46h-v09-p1-plan.", dir=output_parent))
