#!/usr/bin/env python3
"""Select audited PSP content and extend the existing ES-DE media manifest."""
...
def generate(rom_root: Path, base_media: Path, output_root: Path) -> tuple[int, int, int]:
    ...
    psp_root = rom_root / "psp"
    if psp_root.is_symlink() or not psp_root.is_dir():
        raise ValueError("PSP root must be a real directory")

    audit = json.loads(AUDIT.read_text(encoding="utf-8"))
    expected: dict[str, dict[str, object]] = {}
    for entry in audit["content"]:
        relative = safe_relative(entry["path"])
        if relative.parts[0] != "psp" or len(relative.parts) < 2:
            raise ValueError(f"invalid audited PSP path: {relative}")
        name = PurePosixPath(*relative.parts[1:]).as_posix()
        path = require_file(psp_root.joinpath(*relative.parts[1:]), rom_root, "PSP content")
        if path.stat().st_size != entry["size"] or sha256(path) != entry["sha256"]:
            raise ValueError(f"audited PSP content changed: {name}")
        if name in expected:
            raise ValueError(f"duplicate audited PSP content: {name}")
        expected[name] = entry

    source = psp_root / audit["original_gamelist"]["path"].split("/", 1)[1]
    require_file(source, rom_root, "PSP gamelist")
    tree = ET.parse(source)
    root = tree.getroot()
    if root.tag != "gameList":
        raise ValueError("unexpected PSP gamelist root")
    retained: dict[str, ET.Element] = {}
    for game in list(root.findall("game")):
        name = safe_relative(game.findtext("path") or "").as_posix()
        if name in retained:
            raise ValueError(f"duplicate PSP gamelist entry: {name}")
        if name in expected:
            retained[name] = game
        else:
            root.remove(game)
    if set(retained) != set(expected):
        raise ValueError("PSP gamelist and audited content differ")
    ...
    for name, game in retained.items():
        ...
        for xml_name, destination_kind in MEDIA.items():
            ...
            media_path = require_file(psp_root.joinpath(*media.parts), rom_root, "PSP media")
            target = (
                PurePosixPath("psp")
                / destination_kind
                / rom.with_suffix("").parent
                / (rom.stem + media_path.suffix.lower())
            ).as_posix()
            source_path = "/roms/psp/" + media.as_posix()
    ...
    write_tree(tree, output_root / "gamelist.psp.xml")
...
def main() -> int:
    ...
    print(
        f"R46H_PPSSPP_GAMELIST result=pass games={games} "
