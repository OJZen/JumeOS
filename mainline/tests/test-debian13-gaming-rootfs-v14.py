#!/usr/bin/env python3
"""Focused host gates for the Flycast R46H gaming p2 v0.14."""
...
BUILDER_PATH = REPO / "mainline/scripts/build-debian13-gaming-rootfs-v14.py"
SPEC = importlib.util.spec_from_file_location("debian13_gaming_rootfs_v14", BUILDER_PATH)
assert SPEC is not None and SPEC.loader is not None
WRAPPER = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = WRAPPER
SPEC.loader.exec_module(WRAPPER)
BUILDER = WRAPPER.BASE
ROOT = REPO / "mainline/rootfs-debian13-gaming-v14"
IMAGE_TOOL = ROOT / "image-in-container.sh"
FLYCAST = REPO / "mainline/gaming-flycast"
...
class Debian13GamingRootfsV14Tests(unittest.TestCase):
    def test_identity_is_exact_v13_successor(self) -> None:
        self.assertEqual(BUILDER.ARTIFACT_ID, "debian13-p2-gaming-v0.14")
        self.assertEqual(BUILDER.BASE_FS_UUID, "d3130013-46a4-4d56-9001-000000000013")
        self.assertEqual(BUILDER.FS_UUID, "d3130014-46a4-4d56-9001-000000000014")
        self.assertEqual(BUILDER.FS_LABEL, "R46H_GAMING_V14")
        self.assertEqual(
        ...
        self.assertEqual(BUILDER.EXTRA_BUILD_INFO["consolidated_media_link_count"], "6115")
        self.assertEqual(BUILDER.EXTRA_BUILD_INFO["consolidated_media_link_delta"], "28")
        self.assertEqual(BUILDER.EXTRA_BUILD_INFO["flycast_host_load_samples"], "14")
    ...
    def test_composite_configs_are_exact(self) -> None:
        ...
        systems = inputs["es-de-systems.v13.xml"].read_bytes().replace(
            b"</systemList>\n",
            (FLYCAST / "es-system.dreamcast.xml").read_bytes() + b"</systemList>\n",
        )
        links = inputs["system-links.v13.tsv"].read_bytes() + (
            FLYCAST / "system-link.dreamcast.tsv"
        ).read_bytes()
        options = inputs["core-options.v13.cfg"].read_bytes() + (
            FLYCAST / "core-options.cfg"
        ).read_bytes()
        ...
        self.assertEqual(
            sha256(inputs["es-de-retroarch.v13.cfg"]), expected["RETROARCH_APPEND_SHA256"]
        )
        self.assertEqual(len(ET.fromstring(systems).findall("system")), 13)
    ...
    def test_generated_runtime_identities_are_reproducible(self) -> None:
        ...
        command = [
            ...
            "-v", f"core_hash={env['FLYCAST_CORE_SHA256']}",
            "-f", str(FLYCAST / "transform-es-de-runner.awk"),
            str(inputs["r46h-es-de-ui.v13"]),
        ]
        ...
        screenshot = inputs["r46h-screenshot.v13"].read_bytes().replace(
        ...
        firstboot = inputs["r46h-firstboot.v13"].read_bytes().replace(b"v0.13", b"v0.14")
        smoke = inputs["r46h-rootfs-smoke.v13"].read_bytes().replace(
            BUILDER.BASE_FS_UUID.encode(), BUILDER.FS_UUID.encode()
        ).replace(b"v0.13", b"v0.14")
    ...
    def test_media_manifest_is_the_exact_twenty_eight_link_successor(self) -> None:
        manifest = WRAPPER.EXTRA_INPUTS[2][1].read_text(encoding="utf-8").splitlines()
        self.assertEqual(len(manifest), 6116)
        self.assertEqual(manifest, sorted(set(manifest)))
        self.assertEqual(sum(line.startswith("dreamcast/") for line in manifest), 28)
        old = REPO / "mainline/out/r46h-gaming-ppsspp-content-v0.1/legacy-media-links.tsv"
        retained = [line for line in manifest if not line.startswith("dreamcast/")]
    ...
    def test_image_overlay_is_bounded_and_networkless(self) -> None:
        ...
        for required in (
            "base already contains the Flycast core",
        ...
        self.assertIn("INDEPENDENT-FLYCAST-VERIFY.txt", BUILDER.REQUIRED_STAGE_FILES)
