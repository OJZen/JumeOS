#!/bin/bash
# Pure BOOT-directory transaction helpers for the R46H v0.15 promotion.
...
readonly R46H_PREPARE_JOURNAL=.r46h-v15-prepare.journal
readonly R46H_ACTIVATE_JOURNAL=.r46h-v15-activate.journal
readonly R46H_ROLLBACK_JOURNAL=.r46h-v15-rollback.journal
readonly R46H_IMAGE_STAGE=.Image.mainline-v0.15-gaming-product.gz.r46h-stage
readonly R46H_DTB_STAGE=.rk3326-r46h-mainline-v0.15-gaming-product.dtb.r46h-stage
readonly R46H_BOOT_STAGE=.boot.ini.v0.15-gaming-product.r46h-stage
readonly R46H_ACTIVE_STAGE=.boot.ini.v0.15-gaming-product.activate-stage
readonly R46H_BASE_TOP_LEVEL_COUNT=25
...
r46h_tx_check_file() {
  local path=$1 expected_size=$2 expected_hash=$3 label=$4 metadata
  [[ -f "$path" && ! -L "$path" ]] || r46h_tx_fail "unsafe or missing ${label}" || return
  metadata=$(stat -c '%h' "$path" 2>/dev/null || stat -f '%l' "$path") || \
    r46h_tx_fail "cannot stat ${label}" || return
  [[ "$metadata" == 1 ]] || r46h_tx_fail "unexpected link count for ${label}" || return
...
r46h_tx_prepare_journal_bytes() {
  printf '%s\n' \
    ...
    'active_release=6.12.99-r46h-mainline-v0.10-adc-full-range' \
...
r46h_tx_rollback_journal_bytes() {
  printf '%s\n' \
    ...
    'active_release=6.12.99-r46h-mainline-v0.15-gaming-product' \
...
r46h_tx_is_base_name() {
  case "$1" in
    .Spotlight-V100|.console|Image|Image.mainline-v0.10-adc-full-range.gz|\
    Image.mainline-v0.8-bootloader-handoff.gz|'System Volume Information'|\
    USE_DTB_SELECT_TO_SELECT_DEVICE|arkos4clone-uboot.dtb|boot.ini|\
    boot.ini.v0.10-adc-full-range|boot.ini.v0.8-bootloader-handoff|boot.ini.vendor|\
    boot.log|clone_log.txt|consoles|dtb_selector_linux32|dtb_selector_macos|\
    dtb_selector_win32.exe|error.log|firstboot.sh|logo.bmp|rk3326-r46h-linux.dtb|\
...
r46h_tx_is_target_name() {
  case "$1" in
    "$R46H_V15_IMAGE_NAME"|"$R46H_V15_DTB_NAME"|"$R46H_V15_BOOT_NAME") return 0 ;;
    *) return 1 ;;
  esac
}
...
r46h_tx_verify_base_types() {
  local boot=$1 name
  for name in consoles 'System Volume Information' .Spotlight-V100; do
  ...
  for name in \
    ...
    "$R46H_V10_BOOT_NAME" "$R46H_V10_IMAGE_NAME" "$R46H_V10_DTB_NAME"; do
...
r46h_tx_verify_anchors() {
  ...
  case "$active_hash:$active_size:$active_allowed" in
    "$R46H_V10_BOOT_SHA256:$R46H_V10_BOOT_SIZE:v10"|\
    "$R46H_V10_BOOT_SHA256:$R46H_V10_BOOT_SIZE:either"|\
...
r46h_tx_target_count() {
  ...
  [[ ! -e "$boot/$R46H_V15_IMAGE_NAME" && ! -L "$boot/$R46H_V15_IMAGE_NAME" ]] || {
    r46h_tx_check_file "$boot/$R46H_V15_IMAGE_NAME" "$R46H_V15_IMAGE_SIZE" \
      "$R46H_V15_IMAGE_SHA256" 'v0.15 Image' || return
    count=$((count + 1))
  }
  [[ ! -e "$boot/$R46H_V15_DTB_NAME" && ! -L "$boot/$R46H_V15_DTB_NAME" ]] || {
    r46h_tx_check_file "$boot/$R46H_V15_DTB_NAME" "$R46H_V15_DTB_SIZE" \
      "$R46H_V15_DTB_SHA256" 'v0.15 DTB' || return
    count=$((count + 1))
  }
  [[ ! -e "$boot/$R46H_V15_BOOT_NAME" && ! -L "$boot/$R46H_V15_BOOT_NAME" ]] || {
    r46h_tx_check_file "$boot/$R46H_V15_BOOT_NAME" "$R46H_V15_BOOT_SIZE" \
      "$R46H_V15_BOOT_SHA256" 'v0.15 versioned boot script' || return
    count=$((count + 1))
  }
  printf '%s\n' "$count"
}
...
r46h_tx_detect_state() {
  ...
  for name in "$R46H_IMAGE_STAGE" "$R46H_DTB_STAGE" "$R46H_BOOT_STAGE" "$R46H_ACTIVE_STAGE"; do
  ...
  if (( prepare == 1 )); then
    r46h_tx_verify_anchors "$boot" v10 || return
  ...
  if (( activate == 1 )); then
    [[ "$target_count" == 3 ]] || r46h_tx_fail 'activation journal lacks complete target files' || return
    r46h_tx_verify_anchors "$boot" either || return
    case "$active_hash:$active_size" in
      "$R46H_V10_BOOT_SHA256:$R46H_V10_BOOT_SIZE") printf 'activate-partial-v10\n' ;;
  ...
  if (( rollback == 1 )); then
    [[ "$target_count" == 3 ]] || r46h_tx_fail 'rollback journal lacks complete target files' || return
    r46h_tx_verify_anchors "$boot" either || return
    case "$active_hash:$active_size" in
      "$R46H_V10_BOOT_SHA256:$R46H_V10_BOOT_SIZE") printf 'rollback-partial-v10\n' ;;
  ...
  case "$target_count:$active_hash:$active_size" in
    "0:$R46H_V10_BOOT_SHA256:$R46H_V10_BOOT_SIZE")
      r46h_tx_verify_anchors "$boot" v10 || return
      printf 'base\n'
      ;;
    "3:$R46H_V10_BOOT_SHA256:$R46H_V10_BOOT_SIZE")
      r46h_tx_verify_anchors "$boot" v10 || return
      printf 'prepared\n'
      ;;
    "3:$R46H_V15_BOOT_SHA256:$R46H_V15_BOOT_SIZE")
...
r46h_tx_verify_sources() {
  local source=$1
  r46h_tx_check_file "$source/$R46H_V15_IMAGE_NAME" "$R46H_V15_IMAGE_SIZE" \
    "$R46H_V15_IMAGE_SHA256" 'source v0.15 compressed Image' || return
  r46h_tx_check_file "$source/$R46H_V15_DTB_NAME" "$R46H_V15_DTB_SIZE" \
    "$R46H_V15_DTB_SHA256" 'source v0.15 DTB' || return
  r46h_tx_check_file "$source/$R46H_V15_BOOT_NAME" "$R46H_V15_BOOT_SIZE" \
    "$R46H_V15_BOOT_SHA256" 'source v0.15 boot script' || return
}
...
r46h_tx_prepare() {
  ...
  [[ -e "$boot/$R46H_V15_IMAGE_NAME" || -L "$boot/$R46H_V15_IMAGE_NAME" ]] || \
    required_bytes=$((required_bytes + R46H_V15_IMAGE_SIZE))
  [[ -e "$boot/$R46H_V15_DTB_NAME" || -L "$boot/$R46H_V15_DTB_NAME" ]] || \
    required_bytes=$((required_bytes + R46H_V15_DTB_SIZE))
  [[ -e "$boot/$R46H_V15_BOOT_NAME" || -L "$boot/$R46H_V15_BOOT_NAME" ]] || \
    required_bytes=$((required_bytes + R46H_V15_BOOT_SIZE))
  ...
  r46h_tx_publish "$boot" "$source" "$R46H_V15_IMAGE_NAME" "$R46H_IMAGE_STAGE" \
    "$R46H_V15_IMAGE_SIZE" "$R46H_V15_IMAGE_SHA256" 'v0.15 Image' || return
  r46h_tx_fault after-image || return
  r46h_tx_publish "$boot" "$source" "$R46H_V15_DTB_NAME" "$R46H_DTB_STAGE" \
    "$R46H_V15_DTB_SIZE" "$R46H_V15_DTB_SHA256" 'v0.15 DTB' || return
  r46h_tx_fault after-dtb || return
  r46h_tx_publish "$boot" "$source" "$R46H_V15_BOOT_NAME" "$R46H_BOOT_STAGE" \
    "$R46H_V15_BOOT_SIZE" "$R46H_V15_BOOT_SHA256" 'v0.15 versioned boot script' || return
  r46h_tx_fault after-boot || return
  r46h_tx_verify_anchors "$boot" v10 || return
  [[ "$(r46h_tx_target_count "$boot")" == 3 ]] || r46h_tx_fail 'target publication is incomplete' || return
...
r46h_tx_activate() {
  ...
  if [[ "$recovery" == 1 ]]; then
    case "$state" in activate-partial-v10|activate-partial-v15) ;; *)
  ...
  if [[ "$state" == activate-partial-v10 ]]; then
    ...
    cp "$boot/$R46H_V15_BOOT_NAME" "$boot/$R46H_ACTIVE_STAGE" || \
      r46h_tx_fail 'cannot stage active v0.15 boot script' || return
    r46h_tx_check_file "$boot/$R46H_ACTIVE_STAGE" "$R46H_V15_BOOT_SIZE" \
      "$R46H_V15_BOOT_SHA256" 'staged active v0.15 boot script' || return
    r46h_tx_flush || return
    r46h_tx_verify_anchors "$boot" v10 || return
    mv -f "$boot/$R46H_ACTIVE_STAGE" "$boot/$R46H_ACTIVE_BOOT_NAME" || \
      r46h_tx_fail 'cannot publish active v0.15 boot script' || return
...
r46h_tx_rollback() {
  ...
  case "$state" in
    base) return 0 ;;
    prepared|activate-partial-v10|activate-partial-v15|rollback-partial-v10|\
    rollback-partial-v15|activated) ;;
    *) r46h_tx_fail 'rollback requires prepared or activated target files' || return ;;
  esac
  active_size=$(r46h_tx_size "$boot/$R46H_ACTIVE_BOOT_NAME") || return
  active_hash=$(r46h_tx_hash "$boot/$R46H_ACTIVE_BOOT_NAME") || return
  if [[ "$active_hash:$active_size" == "$R46H_V15_BOOT_SHA256:$R46H_V15_BOOT_SIZE" ]]; then
    if [[ "$state" == activated ]]; then
    ...
    cp "$boot/$R46H_V10_BOOT_NAME" "$boot/$R46H_ACTIVE_STAGE" || \
      r46h_tx_fail 'cannot stage v0.10 rollback script' || return
    r46h_tx_check_file "$boot/$R46H_ACTIVE_STAGE" "$R46H_V10_BOOT_SIZE" \
      "$R46H_V10_BOOT_SHA256" 'staged v0.10 rollback script' || return
    r46h_tx_flush || return
    r46h_tx_verify_anchors "$boot" v15 || return
    mv -f "$boot/$R46H_ACTIVE_STAGE" "$boot/$R46H_ACTIVE_BOOT_NAME" || \
      r46h_tx_fail 'cannot publish v0.10 rollback script' || return
