...
readonly FILTER_GENERATOR_SOURCE=$SOURCE/mainline/gaming-fbneo-full/generate-filtered-gamelists.py
readonly AUDIT_SOURCE=$SOURCE/mainline/gaming-fbneo-full/content-audit.json
readonly BASE_SYSTEMS_SOURCE=$SOURCE/mainline/gaming-es-de/es_systems.xml
readonly ARCADE_FRAGMENT_SOURCE=$SOURCE/mainline/gaming-fbneo-full/es-system.arcade.xml
readonly CPS1_FRAGMENT_SOURCE=$SOURCE/mainline/gaming-fbneo-full/es-system.cps1.xml
readonly CPS23_FRAGMENT_SOURCE=$SOURCE/mainline/gaming-fbneo-full/es-systems.cps23.xml
readonly BASE_LINKS_SOURCE=$SOURCE/mainline/gaming-es-de/system-links.tsv
readonly ARCADE_LINK_SOURCE=$SOURCE/mainline/gaming-fbneo-full/system-link.arcade.tsv
readonly CPS1_LINK_SOURCE=$SOURCE/mainline/gaming-fbneo-full/system-link.cps1.tsv
readonly CPS23_LINK_SOURCE=$SOURCE/mainline/gaming-fbneo-full/system-links.cps23.tsv
readonly PRODUCT_DOC_SOURCE=$SOURCE/mainline/rootfs-debian13-gaming-v12/PRODUCT.md
...
readonly FILTER_GENERATOR_SOURCE_SHA256=cf2f03a25fc15fb0a1d26e17fb162189837b4e38c9d33584497b1d97efcb9691
readonly AUDIT_SOURCE_SHA256=04e21c2b82f9b3ce5aac4b2460e939c8d173b2f34fb73bb9bfe9f600e776c267
readonly BASE_SYSTEMS_SOURCE_SHA256=1f451723d4ce1062157eb0e041ff4c098c5adea98c61e87ddf3cb784dfe630f0
readonly ARCADE_FRAGMENT_SOURCE_SHA256=1fbe3e50aaca9c84b2bd2f0a66b41f170c5adcbde8580c8215b60b8c3fbc4711
readonly CPS1_FRAGMENT_SOURCE_SHA256=5ff352b7e5966883a93539a5af92baf4e594d50f881f7ec146c3edb7d110c138
readonly CPS23_FRAGMENT_SOURCE_SHA256=8d3278a9e477c39e4036eaf7d643a66bb99e015036d64bf18c5e376dea2bc235
readonly BASE_LINKS_SOURCE_SHA256=bd5e496ab9bb54f0b9d45b461262b70fd5758d763ea4ff4fb9d06c26c936d73d
readonly ARCADE_LINK_SOURCE_SHA256=058d05441005abd21fced96259c949f469bba10f8bb455d6eb21bcfc05d31e6f
readonly CPS1_LINK_SOURCE_SHA256=c994ef9ffe8e41e9f33e6b6beb15feae7b0e5c59f54955689b95a8f69549e904
readonly CPS23_LINK_SOURCE_SHA256=e2cbb004c0076ea1c1cc9b6bebc83abe011b690b29dd9fd259c476ba2f0c41ec
readonly PRODUCT_DOC_SHA256=817baf3e52880de562942e4b39796551231d42fef5c12973561af374d2d90bd3
readonly BASE_MEDIA_LINKS_SHA256=4903e8bdc5165cbb6bb993dd2632c2e8ec8b2608514c5a5f74e24d9266786cf9
...
verify_inputs() {
  ...
  expect_size_sha256 "$INPUTS/legacy-media-links.tsv" 447780 \
    "$MEDIA_LINKS_SHA256" 'expanded media manifest'
  expect_size_sha256 "$INPUTS/gamelist.cps2.xml" 15702 \
    "$FILTERED_CPS2_GAMELIST_SHA256" 'filtered CPS2 gamelist'
  expect_size_sha256 "$INPUTS/gamelist.cps3.xml" 3563 \
    "$FILTERED_CPS3_GAMELIST_SHA256" 'filtered CPS3 gamelist'
  expect_size_sha256 "$INPUTS/es_settings.v11.xml" 1199 \
    "$BASE_SETTINGS_SHA256" 'v0.11 ES-DE settings input'
  expect_size_sha256 "$INPUTS/r46h-firstboot.v11" 837 \
    "$BASE_FIRSTBOOT_SHA256" 'v0.11 firstboot input'
  expect_size_sha256 "$INPUTS/r46h-rootfs-smoke.v11" 8295 \
    "$BASE_ROOTFS_SMOKE_SHA256" 'v0.11 smoke input'
  members=$(find "$INPUTS" -mindepth 1 -maxdepth 1 -printf '%f\n' | LC_ALL=C sort)
  [[ $members == $'es_settings.v11.xml\ngamelist.cps2.xml\ngamelist.cps3.xml\nlegacy-media-links.tsv\nr46h-firstboot.v11\nr46h-rootfs-smoke.v11' ]] || \
  ...
  expect_sha256 "$FILTER_GENERATOR_SOURCE" "$FILTER_GENERATOR_SOURCE_SHA256" \
    'filtered gamelist generator source'
  expect_sha256 "$AUDIT_SOURCE" "$AUDIT_SOURCE_SHA256" 'content audit source'
  expect_sha256 "$BASE_SYSTEMS_SOURCE" "$BASE_SYSTEMS_SOURCE_SHA256" \
    'base systems source'
  expect_sha256 "$ARCADE_FRAGMENT_SOURCE" "$ARCADE_FRAGMENT_SOURCE_SHA256" \
    'Arcade system fragment'
  expect_sha256 "$CPS1_FRAGMENT_SOURCE" "$CPS1_FRAGMENT_SOURCE_SHA256" \
    'CPS1 system fragment'
  expect_sha256 "$CPS23_FRAGMENT_SOURCE" "$CPS23_FRAGMENT_SOURCE_SHA256" \
    'CPS2/CPS3 system fragment'
  expect_sha256 "$BASE_LINKS_SOURCE" "$BASE_LINKS_SOURCE_SHA256" 'base links source'
  expect_sha256 "$ARCADE_LINK_SOURCE" "$ARCADE_LINK_SOURCE_SHA256" \
    'Arcade link source'
  expect_sha256 "$CPS1_LINK_SOURCE" "$CPS1_LINK_SOURCE_SHA256" 'CPS1 link source'
  expect_sha256 "$CPS23_LINK_SOURCE" "$CPS23_LINK_SOURCE_SHA256" \
    'CPS2/CPS3 link source'
...
verify_base() {
  ...
  path_is_absent "$MEDIA_ROOT/cps2" || die 'base already contains CPS2 media links'
  path_is_absent "$MEDIA_ROOT/cps3" || die 'base already contains CPS3 media links'
  path_is_absent "$GAMELIST_ROOT" || die 'base already contains p2 gamelists'
  path_is_absent "$CPS2_LINK" || die 'base already contains CPS2 system link'
  path_is_absent "$CPS3_LINK" || die 'base already contains CPS3 system link'
...
prepare_system_files() {
  ...
  ' "$BASE_SYSTEMS_SOURCE" "$ARCADE_FRAGMENT_SOURCE" "$CPS1_FRAGMENT_SOURCE" \
    "$CPS23_FRAGMENT_SOURCE" > "$WORK/es-systems.v12"
  expect_sha256 "$WORK/es-systems.v12" "$ES_DE_SYSTEMS_SHA256" \
    'v0.12 ES-DE systems'
  cat "$BASE_LINKS_SOURCE" "$ARCADE_LINK_SOURCE" "$CPS1_LINK_SOURCE" \
    "$CPS23_LINK_SOURCE" > "$WORK/system-links.v12"
  expect_sha256 "$WORK/system-links.v12" "$SYSTEM_LINKS_SHA256" \
    'v0.12 system links'
}

prepare_settings() {
  [[ $(grep -Fxc '<bool name="LegacyGamelistFileLocation" value="true" />' \
    "$INPUTS/es_settings.v11.xml") == 1 ]] || die 'legacy gamelist setting changed'
  ! grep -Fq 'name="ShowHiddenGames"' "$INPUTS/es_settings.v11.xml" || \
    die 'base settings already define hidden-game visibility'
  awk '{
    if ($0 == "<bool name=\"LegacyGamelistFileLocation\" value=\"true\" />")
      $0 = "<bool name=\"LegacyGamelistFileLocation\" value=\"false\" />"
    print
    if ($0 == "<bool name=\"NavigationSounds\" value=\"false\" />")
      print "<bool name=\"ShowHiddenGames\" value=\"false\" />"
  }' "$INPUTS/es_settings.v11.xml" > "$WORK/es_settings.v12.xml"
  expect_sha256 "$WORK/es_settings.v12.xml" "$SETTINGS_SHA256" \
    'v0.12 ES-DE settings'
}
...
prepare_runner() {
  ...
  ' "$RUNNER_SOURCE" > "$WORK/r46h-es-de-ui.v12"
  chmod 0755 "$WORK/r46h-es-de-ui.v12"
  expect_sha256 "$WORK/r46h-es-de-ui.v12" "$ES_DE_RUNNER_SHA256" 'v0.12 ES-DE runner'
  ...
  sed \
    ...
    "$SCREENSHOT_SOURCE" > "$WORK/r46h-screenshot.v12"
  chmod 0755 "$WORK/r46h-screenshot.v12"
  expect_sha256 "$WORK/r46h-screenshot.v12" "$SCREENSHOT_SHA256" 'v0.12 screenshot helper'
}

prepare_identity_tools() {
  sed 's/debian13-p2-gaming-v0\.11/debian13-p2-gaming-v0.12/g' \
    "$INPUTS/r46h-firstboot.v11" > "$WORK/r46h-firstboot.v12"
  chmod 0755 "$WORK/r46h-firstboot.v12"
  expect_sha256 "$WORK/r46h-firstboot.v12" "$FINAL_FIRSTBOOT_SHA256" 'v0.12 firstboot tool'
  sed \
    -e "s/$BASE_FS_UUID/$FS_UUID/g" \
    -e 's/debian13-p2-gaming-v0\.11/debian13-p2-gaming-v0.12/g' \
    "$INPUTS/r46h-rootfs-smoke.v11" > "$WORK/r46h-rootfs-smoke.v12"
  chmod 0755 "$WORK/r46h-rootfs-smoke.v12"
  expect_sha256 "$WORK/r46h-rootfs-smoke.v12" "$FINAL_ROOTFS_SMOKE_SHA256" \
    'v0.12 rootfs smoke tool'
}

prepare_media_tree() {
  local destination source system target total=0 cps2=0 cps3=0
  ...
  while IFS=$'\t' read -r destination source; do
    ...
    total=$((total + 1))
    case $destination in
      cps2/*) system=cps2; cps2=$((cps2 + 1)) ;;
      cps3/*) system=cps3; cps3=$((cps3 + 1)) ;;
      *) continue ;;
    esac
  ...
  [[ $total == 6082 && $cps2 == 62 && $cps3 == 12 ]] || \
    die 'expanded media manifest counts changed'
  [[ $(find "$MEDIA_TREE" -type l | wc -l) == 74 ]] || \
...
prepare_receipts() {
  cat > "$WORK/es-de-receipt.v12" <<EOF
...
filtered_cps2_gamelist_sha256=$FILTERED_CPS2_GAMELIST_SHA256
filtered_cps3_gamelist_sha256=$FILTERED_CPS3_GAMELIST_SHA256
...
composition=offline-filtered-cps-successor-p2-v0.12
EOF
  chmod 0600 "$WORK/es-de-receipt.v12"
  expect_sha256 "$WORK/es-de-receipt.v12" "$ES_DE_RECEIPT_SHA256" 'v0.12 ES-DE receipt'
...
es_de_settings_sha256=$SETTINGS_SHA256
...
prepare_manifests() {
  : > "$DIRECT"
  add_direct "$WORK/es-systems.v12" "$SYSTEMS" 0100644 0 0 replace
  add_direct "$WORK/system-links.v12" "$ES_DE_STATE/system-links.tsv" 0100400 0 0 replace
  add_direct "$WORK/es_settings.v12.xml" "$SETTINGS" 0100600 1000 1000 replace
  add_direct "$INPUTS/gamelist.cps2.xml" "$GAMELIST_ROOT/cps2/gamelist.xml" \
    0100644 1000 1000 new
  add_direct "$INPUTS/gamelist.cps3.xml" "$GAMELIST_ROOT/cps3/gamelist.xml" \
    0100644 1000 1000 new
  add_direct "$WORK/r46h-es-de-ui.v12" "$RUNNER" 0100755 0 0 replace
  add_direct "$WORK/r46h-screenshot.v12" "$SCREENSHOT" 0100755 0 0 replace
  add_direct "$WORK/es-de-receipt.v12" "$ES_DE_RECEIPT" 0100600 0 0 replace
  ...
  add_direct "$WORK/r46h-firstboot.v12" "$FIRSTBOOT" 0100755 0 0 replace
  add_direct "$WORK/r46h-rootfs-smoke.v12" "$ROOTFS_SMOKE" 0100755 0 0 replace
...
apply_overlay() {
  ...
  local link node relative system target
  : > "$commands"
  printf 'mkdir %s\n' "$(quoted "$GAMELIST_ROOT")" >> "$commands"
  emit_metadata "$GAMELIST_ROOT" 0040755 1000 1000 >> "$commands"
  for system in nes famicom gb gbc gba nds neogeo arcade cps1 cps2 cps3; do
    destination=$GAMELIST_ROOT/$system
  ...
  for link in "$CPS2_LINK" "$CPS3_LINK"; do
    target=/roms/${link##*/}
    printf 'symlink %s %s\n' "$(quoted "$link")" "$(quoted "$target")" >> "$commands"
    emit_metadata "$link" 0120777 1000 1000 >> "$commands"
  done
...
verify_unchanged_files() {
  ...
  printf 'R46H_V12_SYSTEMD_VERIFY_RESULT=pass\n' > "$EVIDENCE/SYSTEMD-VERIFY.txt"
  printf 'R46H_V12_UDEV_VERIFY_RESULT=pass\n' > "$EVIDENCE/UDEV-VERIFY.txt"
}
...
verify_gamelists() {
  ...
  for system in nes famicom gb gbc gba nds neogeo arcade cps1; do
    ...
    target=/roms/$system/gamelist.xml
    stat_output=$(debugfs_output "stat \"$destination\"")
    printf '%s\n' "$stat_output" >> "$EVIDENCE/DEBUGFS-V12.txt"
    grep -Fq "Fast link dest: \"$target\"" <<< "$stat_output" || \
      die "gamelist link target mismatch: $system"
    grep -Eq 'User: +1000 +Group: +1000 ' <<< "$stat_output" || \
      die "gamelist link owner mismatch: $system"
  done
  [[ $(grep -Fxc '    <hidden>true</hidden>' "$INPUTS/gamelist.cps2.xml") == 5 ]] || \
    die 'filtered CPS2 hidden count mismatch'
  [[ $(grep -Fxc '    <hidden>true</hidden>' "$INPUTS/gamelist.cps3.xml") == 3 ]] || \
    die 'filtered CPS3 hidden count mismatch'
  grep -Fqx '<bool name="LegacyGamelistFileLocation" value="false" />' \
    "$WORK/es_settings.v12.xml" || die 'legacy gamelist priority remains enabled'
  grep -Fqx '<bool name="ShowHiddenGames" value="false" />' \
    "$WORK/es_settings.v12.xml" || die 'failed CPS games remain visible'
}
...
verify_final() {
  ...
  printf 'R46H_V12_E2FSCK_RESULT=pass\n' >> "$EVIDENCE/E2FSCK.txt"
  ...
  bash -n "$WORK/r46h-es-de-ui.v12" "$WORK/r46h-screenshot.v12" \
    "$WORK/r46h-firstboot.v12" "$WORK/r46h-rootfs-smoke.v12"
...
R46H_V12_PRODUCT artifact_id=$ARTIFACT_ID frontend=ES-DE-3.4.1-r51 systems=11 media_links=6082
R46H_V12_PRODUCT media_delta=74 cps2=57/62 cps3=9/12 hidden=8 roms=external-read-only-p3
R46H_V12_PRODUCT_VERIFY_RESULT=pass
EOF
  printf 'R46H_V12_IMAGE_VERIFY_RESULT=pass\n' >> "$EVIDENCE/DEBUGFS-V12.txt"
}
...
main() {
  ...
  for name in IMAGE_NAME IMAGE_SIZE FS_TAIL_SIZE BASE_IMAGE_SHA256 BASE_FS_UUID \
    ...
    ES_DE_RECEIPT_SHA256 ES_DE_RUNNER_SHA256 ES_DE_SYSTEMS_SHA256 \
    FBNEO_FULL_CORE_SHA256 FILTERED_CPS2_GAMELIST_SHA256 \
    FILTERED_CPS3_GAMELIST_SHA256 MEDIA_LINKS_SHA256 SCREENSHOT_SHA256 \
    SETTINGS_SHA256 SYSTEM_LINKS_SHA256; do
    required_value "$name"
  done
  ...
  verify_final
  printf 'PASS: R46H Debian 13 gaming p2 v0.12 image %s completed.\n' "$ACTION"
}
